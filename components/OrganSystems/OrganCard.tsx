...
export const OrganCard: React.FC<OrganCardProps> = ({ system, isActive, onClick, product }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`organ-systems__card group relative flex flex-col items-center justify-center gap-2.5 md:gap-3 p-[24px_12px_20px] md:p-[32px_16px_24px] rounded-[20px] md:rounded-[24px] cursor-pointer min-h-[110px] md:min-h-[130px] overflow-hidden border transition-all duration-[280ms] ease-out
        ${isActive 
          ? isLongevity
    ...
    >
      ...
      {/* Label */}
      <span className={`relative font-['Aeonik'] text-xs md:text-[13px] font-medium text-center leading-[1.3] transition-colors duration-[280ms] ease-out
